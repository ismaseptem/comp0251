...
----------------------------------------------------------------
.rois_series is a NeXTSTEP typedstream (NSArchiver) archive. Rather than fully
resolve the shared-reference table (fiddly and version-dependent), this parser
keys on two invariants observed in Horos exports:

  * every array is encoded as  92 84 93 96 <count>   (NSMutableArray, ref 0x93)
  * every ROI point is an NSString whose text starts with '{', e.g.
    "{129.37, 66.54}"

Structure:
    root = array, one element per MR slice (index 0..N-1)
      slice = array of ROI objects
        ROI  = object containing a `points` array of MyPoint→NSString "{x,y}"
    RECIST crosshairs are two 2-point line ROIs per tumour.

Public API:
    parse_rois_series(path) -> list[list[list[(x,y)]]]
        [slice][roi][point] ; slice index aligns with the DICOM slice order.
"""
