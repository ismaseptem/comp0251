#!/usr/bin/env python3
"""
extracter.py — segment tumours from crosshair annotations in a single image.

Purpose: detect the coloured crosshair pairs (two intersecting lines per tumour),
model each tumour as an ellipse (longer arm = major axis, shorter = minor), and
write an overlay image + binary mask. Detection helpers are reused by build_volume.
...
def segments_actually_cross(s1, s2, tol=18):
    """Return (crosses, intersection_point): True only when the two segments'
    crossing point lies within each segment's extent (± tol pixels of overshoot).
    """
    x1, y1, x2, y2 = s1
    x3, y3, x4, y4 = s2
