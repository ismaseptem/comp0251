#!/usr/bin/env python3
"""
sam2_propagate.py — baseline SAM2 through-plane propagation (no clip).

Purpose: seed SAM2 from the RECIST masks (one object per connected component),
propagate forward + backward per seed frame, filter by area and centroid
proximity, and OR-merge into one binary label volume. Instance IDs are per-slice
and not matched across seeds or slices.

Use:
    python sam2_propagate.py --sam2-dir sam2_test/sam2 --input-dir sam2_input \\
        --label label.nii.gz --output propagated_label.nrrd   # --model small = faster

Core steps:
  propagate — load SAM2, run fwd+bwd per seed, area/proximity filter, OR-merge, write
...
def _object_passes(
    ...
    Returns (keep, centroid|None, area). Enforces two area anchors — seed cap
    (≤ max_area_ratio × seed area) and per-step shrink (≤ shrink_factor × prev_area)
    — plus a per-tumour centroid-drift limit and a min-component-size gate.
    """
...
def filter_seed_results(
    ...
    accepted centroid and area; per-slice rules (see _object_passes): seed-cap and
    per-step shrink on area, per-tumour centroid drift, min_comp_px removal. The
    seed frame is kept unchanged. Returns {frame_idx: binary_bool_mask}.
    """
