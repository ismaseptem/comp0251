...
Purpose: three subcommands sharing the measurement functions —
  recist      count + volume from a .rois_series (modified ellipsoid V=(π/6)·L·S²)
  propagated  count + volume from a propagated label (measured voxels)
  compare     both side by side, with the propagated/RECIST volume ratio
--third-axis {short,mean,sphere} sets the RECIST through-plane assumption; geometry
for px→mm comes from --raw-dir / --geometry (or the propagated label in `compare`).
...
def measure_propagated(label_img, min_vox=1, merge_gap_z=0):
    """(labelmap, [{label, voxels, vol_mm3, z, cy, cx}, ...], voxel_mm3).

    merge_gap_z>0 groups blobs split across a through-plane dropout: each blob is
    extended merge_gap_z slices up/down (z only) *for connectivity only*, so two
    components separated by a gap of up to 2*merge_gap_z empty slices share one
    label. Volumes/centroids are still measured on the ORIGINAL voxels — no tumour
    tissue is invented in the bridged gap.
    """
    arr = (sitk.GetArrayFromImage(label_img) > 0).astype(np.uint8)   # (Z,H,W)
    sx, sy, sz = label_img.GetSpacing()
    voxel_mm3 = sx * sy * sz
    if merge_gap_z > 0:
        zline = np.ones((2 * merge_gap_z + 1, 1, 1), dtype=int)      # dilate z only
        bridged = ndimage.binary_dilation(arr, structure=zline)
        lab_b, n = ndimage.label(bridged, structure=_CONN3D)
        lab = lab_b * arr                                            # keep real voxels
    else:
...
def report_recist(name, tumours, details):
    vmm = _tot(tumours)
    print(f"\n=== {name}  [RECIST] ===")
    print(f"  tumours: {len(tumours):3d}    ellipsoid volume: "
          f"{vmm/MM3_PER_CM3:8.3f} cm³  ({vmm:9.1f} mm³)")
    if details:
        for k, t in enumerate(tumours):
            print(f"    #{k:2d} slot {t['slot']:2d}  L={t['L_mm']:5.1f}mm "
                  f"S={t['S_mm']:5.1f}mm  V={t['vol_mm3']:8.2f} mm³")


def report_propagated(name, comps, details):
    vmm = _tot(comps)
    print(f"\n=== {name}  [PROPAGATED] ===")
    print(f"  tumours: {len(comps):3d}    measured volume : "
          f"{vmm/MM3_PER_CM3:8.3f} cm³  ({vmm:9.1f} mm³)")
    if details:
        for c in sorted(comps, key=lambda c: -c["vol_mm3"]):
            print(f"    comp {c['label']:3d}  {c['voxels']:6d} vox  "
                  f"V={c['vol_mm3']:8.2f} mm³  "
                  f"@ (z={c['z']:.0f}, y={c['cy']:.0f}, x={c['cx']:.0f})")


def compare_case(rois_path, label_path, third_axis, min_vox, merge_gap_z=0):
    ref = sitk.ReadImage(str(label_path))
    recist = measure_recist(rois_path, ref, third_axis)
    lab, comps, _ = measure_propagated(ref, min_vox, merge_gap_z)
    Z, H, W = lab.shape
    for t in recist:
...
def report_compare(name, r):
    ...
    print(f"  RECIST     : {r['n_recist']:3d} tumours   "
          f"{r['recist_vol_mm3']/MM3_PER_CM3:8.3f} cm³ ({r['recist_vol_mm3']:9.1f} mm³, ellipsoid)")
    print(f"  Propagated : {r['n_propagated']:3d} tumours   "
          f"{r['propagated_vol_mm3']/MM3_PER_CM3:8.3f} cm³ ({r['propagated_vol_mm3']:9.1f} mm³, measured)")
...
def main():
    ...
    pp.add_argument("--merge-gap-z", type=int, default=0,
                    help="Bridge through-plane dropouts: merge blobs across up to "
                         "2*N empty slices (N=1 reconnects a 1-2 slice dropout).")
    ...
    pc.add_argument("--min-vox", type=int, default=1)
    pc.add_argument("--merge-gap-z", type=int, default=0,
                    help="Bridge through-plane dropouts (see 'propagated').")
    pc.add_argument("--out")
    ...
    if a.cmd == "recist":
        ...
        for name, lp in _cases_propagated(a):
            _, comps, _ = measure_propagated(sitk.ReadImage(str(lp)), a.min_vox, a.merge_gap_z)
        ...
        for name, rp, lp in _cases_compare(a):
            try:
                r = compare_case(rp, lp, a.third_axis, a.min_vox, a.merge_gap_z)
            except Exception as e:
