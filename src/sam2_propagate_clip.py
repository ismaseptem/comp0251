#!/usr/bin/env python3
"""
sam2_propagate_clip.py — SAM2 propagation with the shrinking-ellipse clip.

Purpose: seed SAM2 from the RECIST masks, propagate through-plane, and clip each
propagated slice to a depth-scaled shrinking copy of the seed ellipse (pixels
outside are removed). Produces the constrained `_clip` pseudo-label volume; the
only other filter is small-component noise removal.

Use:
    python sam2_propagate_clip.py --sam2-dir sam2 --input-dir sam2_input \\
        --label label.nii.gz --output propagated_label_clip.nrrd

Core steps:
  clip_seed_results — intersect each propagated mask with the sphere-tapered seed ellipse
  propagate         — load SAM2, run fwd+bwd per seed frame, clip, write the volumes
