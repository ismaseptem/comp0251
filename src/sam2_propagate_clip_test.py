#!/usr/bin/env python3
"""
sam2_propagate_clip_test.py — RAW SAM2 OUTPUT variant (no clipping)
-----------------------------------------------------------------------
Diagnostic / teaching version of sam2_propagate_clip.py. Same seeding and
per-lesion RECIST depth-bounded SAM2 propagation, but the two post-hoc filters
are DISABLED:
  • the shrinking-ellipse CLIP is turned off — propagated masks are OR-merged
    exactly as SAM2 emits them (leakage included);
  • the final small-component noise removal is turned off.

Use it to visualise the raw propagation step before showing what the clip does.
(clip_scale / min_comp_px CLI args are accepted but ignored in this variant.)

Reads the frames/ and masks/ produced by prepare_sam2.py, runs SAM2 video
propagation (forward + backward), and writes the unfiltered label volume.
...
def propagate(
    ...
    for seed_num, seed_frame_idx in enumerate(annotated_frames):
        ...
        # Instead of intersecting each propagated slice with the shrinking
        # ellipse and removing small components, we OR-merge SAM2's raw masks
        # directly. This shows the unfiltered propagation — including leakage —
        # so the clipping step can be demonstrated separately.
        covered = 0
        for frame_idx, obj_masks in raw.items():
            merged = None
    ...
    # (Original variant removed sub-min_comp_px fragments from propagated
    # frames here; skipped so this test shows unfiltered output.)
