#!/usr/bin/env python3
"""
roi_series_to_volume.py — build a label volume from a Horos .rois_series (vector path).

Purpose: drop-in alternative to build_volume.py that reads the vector ROI export
instead of detecting coloured crosshairs in pixels, so it is immune to zoom/colour/
JPEG issues and recovers small tumours the detector misses. Pairs the two crossing
line ROIs per tumour, models each as an ellipse, and writes a label in raw MR space.
Slice alignment: array index i maps to raw slice i (both sorted by position); use
--flip-z if reversed, --compare to check against an existing label.

Use:
    python roi_series_to_volume.py --rois brac46551b.rois_series \\
        --raw-dir brac46551b_raw/ --output label_vec.nrrd   # optional: --compare label.nrrd
"""
