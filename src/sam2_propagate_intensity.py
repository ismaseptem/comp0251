#!/usr/bin/env python3
"""
sam2_propagate_intensity.py — clip variant plus an intensity-band gate.

Purpose: sam2_propagate_clip.py with one extra appearance filter. Per lesion the
seed core's grayscale [P, 100-P] band is measured; on each propagated frame, mask
pixels outside that band are trimmed and blobs with no in-band core are dropped
(--intensity-min-core rejects a whole blob when too little is in-band). Catches
masks that slid onto differently-bright tissue while barely moving. The gate runs
after the clip, before noise removal. --intensity-pct 0 (default) = OFF, which
reproduces sam2_propagate_clip.py exactly.

Use:
    python sam2_propagate_intensity.py --sam2-dir sam2 --input-dir sam2_input \\
        --label label.nii.gz --output propagated_label_intensity.nrrd \\
        --intensity-pct 2 --intensity-min-core 0.3 --intensity-rim-px 1

Core steps:
  clip_seed_results — geometric clip (shared with sam2_propagate_clip.py)
  intensity gate    — trim mask pixels outside the seed's grayscale band
  propagate         — fwd+bwd per seed, clip, intensity gate, noise removal, write
"""
...
def seed_intensity_band(gray: np.ndarray, comp_mask: np.ndarray, pct: float):
    """Intensity band [lo, hi] of the seed core: the (pct, 100-pct) percentiles of
    the MR grayscale INSIDE the seed component. None when disabled or empty.

    This is "what this lesion looks like". prepare_sam2 windows every frame with the
    SAME global 1st/99th-percentile bounds (see its `--- Global intensity window ---`
    step), so a grayscale value means the same thing on every slice and the band is
    directly comparable to intensities on neighbouring frames. If that windowing ever
    becomes per-slice, this whole filter silently breaks.
    """
    if pct <= 0:
        return None
...
def apply_intensity_band(clipped: np.ndarray, gray: np.ndarray, band,
                         rim_px: int, min_core_frac: float = 0.0) -> np.ndarray:
    """Trim a propagated mask to pixels that still look like the seed.

      1. inband = mask pixels whose grayscale is inside the seed band [lo, hi].
      2. core   = the in-band part of the mask (the plausible tumour).
      3. keep core plus a rim_px dilation intersected back with the mask, so a
         thin real tumour rim survives.

    A blob with no in-band core is dropped. min_core_frac additionally drops a blob
    whose in-band fraction is below the threshold (0 disables that test).
    """
    if band is None or not clipped.any():
        return clipped
    lo, hi = band
    core = clipped & (gray >= lo) & (gray <= hi)
    if not core.any():
        return np.zeros_like(clipped)                     # pure leak → drop
    if min_core_frac > 0 and core.sum() < min_core_frac * clipped.sum():
        return np.zeros_like(clipped)                     # mostly off-appearance → drop
    if rim_px > 0:
        k = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,
                                      (2 * rim_px + 1, 2 * rim_px + 1))
        return cv2.dilate(core.astype(np.uint8), k).astype(bool) & clipped
    return core
...
def clip_seed_results(
    ...
    kept as-is; the depth window (obj_depth) bounds propagation; the intensity gate
    and noise removal run after the clip. Returns {frame_idx: binary_bool_mask}.
    """
    result: dict = {}
    ...
    for lid, ell in enumerate(seed_ellipses, start=1):
        ...
        for frame_idx, obj_masks in raw.items():
            ...
            # Appearance gate, AFTER the geometric clip. Geometry and motion cannot
            # see a mask that has slid onto tissue of a different brightness while
            # barely moving — that is the failure mode this catches.
            if seed_bands is not None and frames_gray is not None:
                clipped = apply_intensity_band(
                    clipped, frames_gray[frame_idx], seed_bands.get(lid),
                    intensity_rim_px, intensity_min_core)
...
def propagate(
    ...
    if intensity_pct > 0:
        print(f"Intensity band   : keep pixels inside the seed core's "
              f"[{intensity_pct:g}, {100 - intensity_pct:g}] percentile grayscale band "
              f"(+{intensity_rim_px}px rim"
              + (f", drop blobs <{intensity_min_core:g} in-band" if intensity_min_core > 0 else "")
              + ")")
    else:
        print("Intensity band   : OFF")
    ...
    # Grayscale MR per frame for the intensity band. Indexed the same way SAM2
    # indexes the video (sorted filenames), so frames_gray[i] is SAM2's frame i.
    frames_gray = None
    if intensity_pct > 0:
        frame_files = sorted(
            p for p in frames_dir.iterdir() if p.suffix in (".jpg", ".jpeg", ".png"))
    ...
    for seed_num, seed_frame_idx in enumerate(annotated_frames):
        ...
        # Per-lesion appearance band, measured on THIS seed frame's annotation.
        seed_bands = None
        if frames_gray is not None:
            seed_bands = {lid: seed_intensity_band(frames_gray[seed_frame_idx], m,
                                                   intensity_pct)
                          for lid, m in enumerate(comp_masks, start=1)}
            band_str = ", ".join(f"obj{lid}=[{b[0]:.0f},{b[1]:.0f}]"
                                 for lid, b in sorted(seed_bands.items()) if b)
        ...
        # obj_stop_d[lid][direction] = last depth at which SAM2 was still holding
        # THIS lesion. Always filled in: the dropout half of the continuity gate is
        # unconditional. max_step_velocity <= 0 drops only the velocity test and the
        # pole extension, so the taper keeps the width the measured radius implies.
        ...
        clipped = clip_seed_results(raw, seed_frame_idx, seed_ellipses, obj_depth,
                                    ...
                                    frames_gray=frames_gray, seed_bands=seed_bands,
                                    intensity_rim_px=intensity_rim_px,
                                    intensity_min_core=intensity_min_core)
        covered = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--intensity-pct", type=float, default=0.0, metavar="P",
                        help="Intensity-band appearance gate (default: 0 = OFF, which "
                             "reproduces sam2_propagate_clip.py exactly). For each "
                             "lesion the seed core's grayscale [P, 100-P] percentile "
                             "band is measured on its seed frame; on every propagated "
                             "frame, mask pixels outside that band are trimmed and "
                             "blobs with no in-band core are dropped. Try 2-5. This is "
                             "the only gate that catches a mask which slid onto "
                             "brighter/darker tissue WITHOUT moving much — geometry "
                             "and --max-step-velocity are both blind to it. Frames are "
                             "globally windowed by prepare_sam2, so the band is "
                             "comparable across slices.")
    parser.add_argument("--intensity-rim-px", type=int, default=2, metavar="N",
                        help="Rim tolerance for --intensity-pct (default: 2). Keeps "
                             "N pixels of legitimately darker/brighter tumour rim "
                             "attached to the in-band core so the band does not "
                             "nibble real lesion boundaries. 0 = strict band.")
    parser.add_argument("--intensity-min-core", type=float, default=0.0, metavar="F",
                        help="Drop a blob outright when its in-band fraction is below "
                             "F (default: 0 = off, trim only). On brac frame 32 the "
                             "false lesion sitting on the bright band is 19-22%% "
                             "in-band while genuine propagation is mostly in-band, so "
                             "0.3-0.5 removes it instead of merely trimming it to its "
                             "core. Raising this makes the gate reject whole blobs "
    ...
    propagate(
        ...
        intensity_min_core=args.intensity_min_core,
    )
