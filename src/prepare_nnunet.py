...
Purpose: for every case, (1) read the raw MR DICOM series with the SAME reader that
built the labels (build_volume.read_dicom_series) so image and label share the exact
voxel grid nnU-Net requires; (2) write imagesTr/<case>_0000.nii.gz + labelsTr/<case>.nii.gz;
(3) verify image↔label geometry and skip mismatches. Also writes dataset.json and a
PATIENT-GROUPED splits_final.json (so a patient never spans train/val of a CV fold).

Use:
    python prepare_nnunet.py --raw-root ~/Downloads/samples \\
        --labels-dir ~/Downloads/output/out --label-suffix _clip \\
        --cases ~/Downloads/output/cases.txt --dataset-id 501 --dataset-name Tumour \\
        --output nnUNet_raw
    # then nnUNetv2_plan_and_preprocess, copy splits_final.json into
    # nnUNet_preprocessed/<Dataset>/, then nnUNetv2_train.
"""
