...
--------------------------------------------------------------------------
Like overlay_val.py, but instead of one montage PNG per case it writes ONE
PNG per slice into a per-sample subfolder, covering EVERY slice of the volume
(not just tumour-bearing ones). Green = SAM2 weak label, red = nnU-Net.
...
Sources (nnU-Net v2 layout):
  ...
  prediction  : <preds>/<case>.nii.gz   (a fold's validation/ or crossval dir)

Usage
-----
  python overlay_slices.py \\
      --images $nnUNet_raw/Dataset501_Tumour/imagesTr \\
      --labels $nnUNet_raw/Dataset501_Tumour/labelsTr \\
      --preds  $nnUNet_results/Dataset501_Tumour/nnUNetTrainer__nnUNetPlans__3d_fullres/crossval_results_folds_0_1_2_3_4 \\
      --out    slice_review

  # only specific cases
  python overlay_slices.py --images ... --labels ... --preds ... \\
      --cases brac52471b_02 dabj191c_00 --out slice_review

  # skip empty slices (no tumour in gt or pred) to save disk
  python overlay_slices.py ... --tumour-only
"""
import argparse
