#!/usr/bin/env python3
"""
sam2_propagate_boxpoint.py — box+point seeding variant.

Purpose: like sam2_propagate.py but SAM2 is prompted with each component's
bounding box + centroid point (add_new_points_or_box) instead of the filled
RECIST ellipse, so it segments from image features rather than inheriting the
ellipse shape. All downstream logic is unchanged, so outputs are comparable.

Use:
    python sam2_propagate_boxpoint.py --sam2-dir sam2 --input-dir sam2_input \\
        --label label.nii.gz --output propagated_label_boxpoint.nrrd  # --model small = faster

Core steps:
  propagate — box+point seed, fwd+bwd per seed, depth/drift/area filter, OR-merge, write
...
def drift_limit_for(radius_px: float, drift_frac: float,
    ...
    limit = drift_frac × radius_px, clamped to [drift_floor, drift_cap] — scales
    with lesion size so the check is scale-invariant.
    """
...
def _object_passes(
    ...
    """Decide whether a propagated object mask is a plausible continuation.

    Returns (keep, centroid|None). Area is capped at max_area_ratio × seed area;
    the per-step centroid drift is thresholded per-tumour (seed_stat["drift_limit"]);
    a min-component-size gate strips sub-threshold fragments.
    """
    ...
    if largest_cc < min_px:
        return False, None
    if int(prop_mask.sum()) > max_area_ratio * seed_stat["area"]:
        return False, None
...
def filter_seed_results(
    ...
    Each object is walked outward from the seed (fwd then bwd), tracking the last
    accepted centroid; per-slice rules (see _object_passes): area ≤ max_area_ratio ×
    seed area, per-tumour centroid drift, min_comp_px removal. The seed frame is
    kept unchanged. Returns {frame_idx: binary_bool_mask}.
    """
    ...
    for lid, ss in enumerate(seed_stats, start=1):
        ...
        # Walk each direction independently, comparing to the last accepted centroid.
        for step in (1, -1):
            ...
            while lo <= f <= hi:
                ...
                if prop_mask is None:          # past this object's recorded range
                    break
                keep, cxy = _object_passes(
                    prop_mask, ss, prev_cxy, max_area_ratio, min_comp_px
                )
                if keep:
                    cleaned = remove_small_components(prop_mask, min_comp_px)
                    if cleaned.any():
                        _add(f, cleaned)
                        prev_cxy = cxy         # advance reference to this accepted slice
                # A failed slice is skipped but does not advance prev_cxy, so the next
                # slice is still measured against the last good position.
                f += step
...
def propagate(
    ...
    print(f"Filters          : max_area_ratio={max_area_ratio}  "
    ...
    for seed_num, seed_frame_idx in enumerate(annotated_frames):
        ...
        # BOX+POINT seeding variant: instead of forcing SAM2 to the filled RECIST
        # ellipse (add_new_mask), prompt it with the ellipse's bounding box plus a
        # positive point at the centroid and let SAM2 segment the tumour from image
        # features. This avoids the elliptical-blob shape bias of the mask prompt.
        # Everything downstream (seed_stats, depth, drift, area filters) is unchanged
        # and still derived from the ellipse, so this is a clean A/B vs sam2_propagate.py.
        predictor.reset_state(inference_state)
        for lid, m in enumerate(comp_masks, start=1):
            ys, xs = np.where(m)
            box = np.array([int(xs.min()), int(ys.min()),
                            int(xs.max()), int(ys.max())], dtype=np.float32)
            point = np.array([[float(xs.mean()), float(ys.mean())]], dtype=np.float32)
            predictor.add_new_points_or_box(
                inference_state, seed_frame_idx, lid,
                points=point,
        ...
        # RECIST depth limit, per lesion: assume roughly spherical, so through-plane
        # extent ≈ in-plane radius each way. depth = radius_mm / slice_thickness_mm
        # (radius = min-enclosing-circle radius). Each object bounded by its own
        # radius; loop bounded by the largest per-object depth.
            ...
            if out_f != seed_frame_idx:
                ...
                for oid, m in frame_masks.items():
                    if oid - 1 >= len(seed_stats):
                        continue
                    ok, cxy = _object_passes(m, seed_stats[oid - 1], prev_cxy[oid],
                                             max_area_ratio, min_comp_px)
                    if ok:
                        prev_cxy[oid] = cxy
                        passed = True
                if passed:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output",     default="propagated_label_boxpoint.nrrd",
                        help="Output NRRD path (default: propagated_label_boxpoint.nrrd)")
