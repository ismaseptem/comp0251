#!/usr/bin/env python3
"""
build_volume.py — build a 3-D label volume from crosshair-annotated ROI DICOMs.

Purpose: detect crosshair annotations in ROI (Secondary Capture) DICOMs and write
a label volume in the raw MR coordinate space. Pass --raw-dir so the label gets the
correct MR spacing/origin/direction and matches prepare_sam2.py frames (without it
the label is written in SC space).

Use:
    python build_volume.py --roi-dir brac46551b_roi/ --raw-dir brac46551b_raw/ \\
        --output label.nrrd
"""
