#!/usr/bin/env python3
"""
prepare_sam2.py — convert raw MR DICOMs + a label into SAM2 video input.

Purpose: read the raw (unannotated) MR DICOM folder and a label built by
build_volume.py, and write frames/<NN>.jpg (raw MR slices as RGB, the SAM2 video),
masks/<NN>.png (binary prompts on annotated slices), and frame_info.csv. --raw-dir
and --label must be in the same MR space; frame index follows the GDCM slice order.

Use:
    python prepare_sam2.py --raw-dir brac46551b_raw/ --label label.nii.gz \\
        --output sam2_input/
"""
