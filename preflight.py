...
------------------------------------------------------------------------
Catches the *silent* export mistakes that the individual pipeline scripts
don't (they only crash on hard errors). Run this first; fix any FAIL before
building the volume.

Checks
------
  RAW series (--raw-dir):
    · is real MR pixel data, not a rendered screenshot
      (MONOCHROME2, >8-bit, SamplesPerPixel==1, Modality MR)
    · ImagePositionPatient / PixelSpacing / SliceThickness present
    · consistent dimensions, no duplicate slice positions
    · lossless transfer syntax

  There are two annotation-ingestion paths; validate whichever you feed the
  builder (you may pass both):

  VECTOR path (--rois, feeds roi_series_to_volume.py)  ← preferred
    We want ONLY straight crossing line pairs (RECIST long + short axis). These
    FAIL the export:
    · the .rois_series file parses as a typedstream (non-empty slots)
    · at least one annotated slice
    · every ROI is a straight 2-point line — a closed oval / circle / rectangle
      or freehand is stored as a many-point polygon and is rejected
    · lines come in pairs (even count) — no lone / unpaired axis
    · each pair actually crosses — no mispaired or parallel (non-crossing) lines
    · ROI coords lie inside the acquisition grid (warn)
    · slot count matches the raw slice count, index i → raw slice i (warn)

  COLOUR path (--roi-dir, feeds build_volume.py)
    · files carry a .dcm extension  (build_z_map needs it)
    · annotations are burned into RGB pixels (SamplesPerPixel==3)
    · ImagePositionPatient present (needed for z-matching)
    · crosshairs are actually detectable (colors within the extracter set)
    · no unsupported annotation shapes — flags closed ROIs (oval / circle /
      polygon / filled) via enclosed area and stray/freehand/arrow marks via
      colour lying off the crosshair arms. (Text labels are a known blind
      spot: short strokes can mimic tiny crosshairs.)
    · lossless transfer syntax (protects annotation hue)

  CROSS-CHECK:
    · colour path — every annotated ROI slice matches a RAW slice by z-position
    · vector path — slot count agrees with the raw slice count
      (the #1 cause of an all-blank / shifted label).

Usage
-----
    # preferred vector path
    ...
    python preflight.py --raw-dir raw/ --roi-dir roi/ --min-arm-len 10
"""
